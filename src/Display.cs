...
namespace ScreenAutoRotate
{
    ...
    public class Display
    {
        ...
        public static (long,long) Position(uint display)
        {
            if (display == 0)
                throw new ArgumentOutOfRangeException("DisplayNumber", display, "First display is 1.");

            DISPLAY_DEVICE d = new DISPLAY_DEVICE();
            DEVMODE dm = new DEVMODE();
            d.cb = Marshal.SizeOf(d);

            if (!NativeMethods.EnumDisplayDevices(null, display - 1, ref d, 0))
                throw new ArgumentOutOfRangeException("DisplayNumber", display, "Number is greater than connected displays.");
        ...
        public static uint Connected()
        {
            ...
            while (true)
            {
                ...
                if (!NativeMethods.EnumDisplayDevices(null, i++, ref d, 0)
                    || (0 == NativeMethods.EnumDisplaySettings(d.DeviceName, NativeMethods.ENUM_CURRENT_SETTINGS, ref dm)))
                {
                    return i - 1;
                }
            }
        ...
        public static bool LandscapeMode(uint display)
        {
            if (display == 0)
                throw new ArgumentOutOfRangeException("DisplayNumber", display, "First display is 1.");
            ...
            if (0 != NativeMethods.EnumDisplaySettings(
                d.DeviceName, NativeMethods.ENUM_CURRENT_SETTINGS, ref dm))
            {
                return dm.dmDisplayOrientation == NativeMethods.DMDO_DEFAULT;
            }
            else
            {
                return false;
        ...
        public static bool Move(uint DisplayNumber, long PositionX, long PositionY)
        {
            if (DisplayNumber == 0)
           ...
           d.DeviceName, NativeMethods.ENUM_CURRENT_SETTINGS, ref dm))
            {
                dm.dmPosition.x = PositionX;
                dm.dmPosition.y = PositionY;
    ...
    internal struct POINTL
    {
        ...
        public POINTL(long positionX, long positionY) : this()
        {
        }
    }
